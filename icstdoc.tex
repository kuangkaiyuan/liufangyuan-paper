...
for Android application developers. It is often used by
...
often require having access to the source code. This requirement limits their application because the obfuscated
code can be removed by the compiler and developers often do not want to to release their source code.
Because of this, a code protection scheme that can be applied
to the compiled Anroid bytecode is highly attractive.
...
Dex bytecode and does not require access to high-level program source or modification of the
compiler or VM. Our approach exploits the two key structures of the Android Dex format
 to provide stronger protection against dynamic code analysis when compared to prior work.
 Firstly, our approach leverages the fact all except floating
...
bytecode will result receive incorrect information. Secondly, our approach obfuscates the program
control flow by inserting opaque predicates before the return instruction of
a function call, so that the program exhibits more complex runtime behavior and
the attacker is unlikely to successfully trace calls to protected functions. %These two techniques, putting together, enable us to
...
\printccsdesc
% no keywords
\keywords{Code obfuscation, Android code protection, Reverse engineering}\\
