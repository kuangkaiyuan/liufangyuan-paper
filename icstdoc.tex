...
code. Our approach exploits the two key structures of the Android Dex stack machine
 for code protection. Firstly, our approach leverages the fact all except floating
...
bytecode will result in wrong operands. Secondly, we obfuscate the program
control flow by inserting opaque predicates before the return instruction of
a function call to protect the application against dynamic analysis. These two techniques, putting together, enable us to
build a new, more powerful
byte-code level protection scheme for Android apps. 
